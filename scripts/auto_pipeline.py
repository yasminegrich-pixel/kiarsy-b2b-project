...
# Make sure the scripts folder is in the path
...
def main(name, url, industry="", region=""):
    ...
    if run_id is None:
        print("❌ No scoring run exists. Run scripts/scorer.py first.")
        return
    ...
    if row:
        ...
        cur.execute(
            "INSERT INTO companies (company_id, company_name, industry, country_region, company_summary) VALUES (%s,%s,%s,%s,%s)",
            (c_id, name, industry, region, text[:2000])
        )
    for vid, vname, tier, quote, e in tags:
        cur.execute(
            "INSERT INTO company_values (company_id, value_id, tier, evidence_summary) VALUES (%s,%s,%s,%s)",
            (c_id, vid, tier, quote)
        )
    conn.commit()
    ...
    for dim in DIMS:
        ...
        for si in range(len(sents)):
            ...
            if 'lo' in sc and 'hi' in sc:
                ...
                pos = float(np.clip((margin+2)/4, 0, 1))
                conf = float(np.clip(abs(margin)/2, 0, 1))
                if conf >= MIN_EVID:
    ...
    cur.execute("""SELECT s.symbol_name, a.similarity 
                   FROM company_symbol_affinity a 
                   JOIN symbols s ON s.symbol_id=a.symbol_id 
                   WHERE a.company_id=%s AND a.run_id=%s 
                   ORDER BY a.similarity DESC LIMIT 5""", (c_id, run_id))
    for r in cur.fetchall():
